...
# Author  : Mohammed Ali
# Version : 4.0.2
# Purpose : dbaascli database create precheck
######################################################
...
Options:
  -p              Run PRIMARY DB create precheck
  -s              Run STANDBY DB create precheck
...
MASTER_LOG="$LOG_DIR/odaa_db_create_precheck_${RUN_MODE}_${RUN_TS}.log"
SUMMARY_FILE="$LOG_DIR/db_create_summary_${RUN_MODE}_${RUN_TS}.txt"

exec > >(tee -a "$MASTER_LOG") 2>&1

echo " "
echo "                      DATABASE CREATE PRECHECK SUMMARY            " | tee "$SUMMARY_FILE"
echo "____________________________________________________________________________________________"
echo "Script      : $SCRIPT_NAME"
echo "Purpose     : dbaascli create database precheck only"
echo "Author      : Mohammed Ali"
echo "Version     : 4.0.2"
echo "Mode        : $RUN_MODE DATABASE CREATE PRECHECK ONLY"
echo "INI         : $INI_FILE"
...
do
  ...
  if [[ "$RUN_MODE" == "PRIMARY" ]]; then
    ...
    TARGET_ORACLE_HOME="${dbHome:-${ORACLE_HOME:-/u02/app/oracle/product/19.0.0.0/dbhome_1}}"
  else
    TARGET_DB_NAME="$dbName"
  ...
  if db_exists "$TARGET_DB_NAME"; then
    echo "⚠️  $TARGET_DB_NAME : ALREADY EXISTS → PRECHECK SKIPPED" | tee -a "$SUMMARY_FILE"
    continue
  fi
  ...
  if [[ "$PRECHECK_RC" -eq 0 ]] && grep -Eqi "success|successfully|passed|completed|prereq" "$PRECHECK_LOG"; then
    ...
    echo "❌ $TARGET_DB_UNIQUE_NAME : PRECHECK FAIL" | tee -a "$SUMMARY_FILE"
  ...
  echo "   DB Log: $PRECHECK_LOG" | tee -a "$SUMMARY_FILE"
