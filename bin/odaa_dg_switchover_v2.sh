...
Usage:
  ...
  $(basename "$0") -p --version-status -d MCK
  $(basename "$0") -s --version-status --all
  $(basename "$0") -p --datapatch-status -d MCK
  $(basename "$0") -s --datapatch-status --all
  $(basename "$0") -p --patch-report --all
  $(basename "$0") -s --patch-report -d POC,MCK
  $(basename "$0") -p --fix-broker -d MCK
  $(basename "$0") -s --fix-broker -d MCK
  $(basename "$0") -p --switchover -d MCK
  $(basename "$0") -s --switchover -d MCK --datapatch
  $(basename "$0") -p --switchover --all --datapatch
...
Actions:
  ...
  --version-status     Show Oracle DB version, OPatch version, datapatch version, registry, and SQL patch status
  --datapatch-status   Show one-line datapatch / SQL patch status
  --patch-report       Show Oracle patch compliance report with PASS/WARNING/FAILED summary
...
Options:
  ...
  --datapatch         Run datapatch after successful switchover on new primary
  -h, --help           Display help
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --version-status) ACTION="VERSION_STATUS"; shift ;;
    --datapatch-status) ACTION="DATAPATCH_STATUS"; shift ;;
    --patch-report) ACTION="PATCH_REPORT"; shift ;;
    ...
    --datapatch) RUN_DATAPATCH="Y"; shift ;;
...
if [[ "$DB_FILTER" == "ALL" && "$ACTION" != "STATUS" && "$ACTION" != "VERSION_STATUS" && "$ACTION" != "DATAPATCH_STATUS" && "$ACTION" != "PATCH_REPORT" ]]; then
  echo
...
print_banner() {
  {
    ...
    echo "Run Datapatch:  $RUN_DATAPATCH"
    echo "INI         :  $INI_FILE"
    echo "Start       :  $(date)"
    printf '%0.s_' {1..155}
    echo
...
show_broker_connect_info() {
  ...
  printf '%0.s_' {1..155} | tee -a "$LOG_FILE"
  echo | tee -a "$LOG_FILE"
...
wait_for_primary_read_write() {
  local dbHome="$1"
  local syspass="$2"
  local connect_alias="$3"
  ...
  log_both "Waiting for database to be PRIMARY and READ WRITE before datapatch..."

  while [[ "$elapsed" -le "$timeout_seconds" ]]; do
    status_out=$("$dbHome/bin/sqlplus" -L -s /nolog <<EOF 2>&1
whenever sqlerror exit failure
connect sys/${syspass}@${connect_alias} as sysdba
...
EOF
    ...
    db_role="$(echo "$status_out" | awk -F'|' 'NF>=2 {gsub(/^ *| *$/, "", $1); print $1; exit}')"
    open_mode="$(echo "$status_out" | awk -F'|' 'NF>=2 {gsub(/^ *| *$/, "", $2); print $2; exit}')"

    log_only "Datapatch wait check: role=${db_role:-UNKNOWN}, open_mode=${open_mode:-UNKNOWN}, elapsed=${elapsed}s"

    if [[ "$db_role" == "PRIMARY" && "$open_mode" == "READ WRITE" ]]; then
      log_both "Database is PRIMARY and READ WRITE."
      return 0
    fi
  ...
  log_both "ERROR: Timeout waiting for PRIMARY READ WRITE before datapatch."
  log_both "Last status output: $status_out"
  return 1
}
...
run_sqlpatch_verify() {
  ...
  verify_out=$("$dbHome/bin/sqlplus" -L -s /nolog <<EOF 2>&1
whenever sqlerror exit failure
connect sys/${syspass}@${connect_alias} as sysdba
...
select to_char(action_time,'YYYY-MM-DD HH24:MI:SS') action_time, action, status, description from dba_registry_sqlpatch order by action_time desc fetch first 10 rows only;
exit;
EOF
) || true

  log_output "DBA_REGISTRY_SQLPATCH VERIFY - $connect_alias" "$verify_out"
  echo "$verify_out" | tee -a "$LOG_FILE"
}

run_post_switchover_datapatch() {
  local dbHome="$1"
  local dbName="$2"
  local targetDBUniqueName="$3"
  local syspass="$4"
  local datapatch_out

  if [[ "$RUN_DATAPATCH" != "Y" ]]; then
    log_both "Datapatch option not enabled. Skipping datapatch."
    return 0
  fi

  if [[ ! -x "$dbHome/OPatch/datapatch" ]]; then
    log_both "ERROR: datapatch not found or not executable: $dbHome/OPatch/datapatch"
    return 1
  fi

  export ORACLE_HOME="$dbHome"
  export PATH="$ORACLE_HOME/bin:$ORACLE_HOME/OPatch:$PATH"
  export ORACLE_SID="${targetDBUniqueName}1"

  log_both ""
  log_both "Starting post-switchover datapatch for $dbName / $targetDBUniqueName"
  log_both "ORACLE_HOME : $ORACLE_HOME"
  log_both "ORACLE_SID  : $ORACLE_SID"
  log_both "TNS_ADMIN   : ${TNS_ADMIN:-NOT_SET}"

  wait_for_primary_read_write "$dbHome" "$syspass" "$targetDBUniqueName" 600 || return 1

  datapatch_out=$("$dbHome/OPatch/datapatch" -verbose 2>&1) || true
  log_output "DATAPATCH - $dbName / $targetDBUniqueName" "$datapatch_out"

  if echo "$datapatch_out" | grep -qiE "SQL Patching tool complete|Datapatch complete"; then
    log_both "Datapatch completed successfully."
    run_sqlpatch_verify "$dbHome" "$syspass" "$targetDBUniqueName"
    return 0
  fi

  log_both "ERROR: Datapatch failed. Check datapatch output in log."
  ERR_LINE="$(first_error_line "$datapatch_out")"
  [[ -n "$ERR_LINE" ]] && log_both "Reason: $ERR_LINE"
  run_sqlpatch_verify "$dbHome" "$syspass" "$targetDBUniqueName" || true
  return 1
}


run_datapatch_status() {
  local dbHome="$1"
  local dbName="$2"
  local connect_alias="$3"
  local syspass="$4"
  local dp_version="UNKNOWN"
  local opatch_version="UNKNOWN"
  local sqlpatch_line=""
  local sql_status="FAILED"
  local db_role="UNKNOWN"
  ...
  local ru_status="N/A"
  local ru_level="N/A"
  local ojvm_patch="N/A"
  local ojvm_status="N/A"
  local last_patch_time="N/A"
  local invalid_count="UNKNOWN"
  local patch_result="WARNING"

  export ORACLE_HOME="$dbHome"
  export PATH="$ORACLE_HOME/bin:$ORACLE_HOME/OPatch:$PATH"

  if [[ -x "$ORACLE_HOME/OPatch/datapatch" ]]; then
    dp_version=$("$ORACLE_HOME/OPatch/datapatch" -version 2>&1 \
      | awk '/SQL Patching tool version/ {print $5; exit}' || true)
    [[ -z "$dp_version" ]] && dp_version="UNKNOWN"
  else
    dp_version="NOT_FOUND"
  fi

  if [[ -x "$ORACLE_HOME/OPatch/opatch" ]]; then
    opatch_version=$("$ORACLE_HOME/OPatch/opatch" version 2>&1 \
      | awk -F': ' '/OPatch Version/ {print $2; exit}' || true)
    [[ -z "$opatch_version" ]] && opatch_version="UNKNOWN"
  else
    opatch_version="NOT_FOUND"
  fi

  sqlpatch_line=$("$ORACLE_HOME/bin/sqlplus" -L -s /nolog <<EOF 2>&1
whenever sqlerror exit failure
connect sys/${syspass}@${connect_alias} as sysdba
set heading off feedback off pages 0 lines 600 trimspool on verify off echo off termout on
select database_role || '|' ||
       open_mode || '|' ||
       nvl((select version_full from product_component_version where product like 'Oracle Database%' and rownum = 1),'UNKNOWN') || '|' ||
       nvl((select value from v\$parameter where name='compatible'),'UNKNOWN') || '|' ||
       nvl((select to_char(patch_id) || '|' || status || '|' || nvl(regexp_substr(description,'[0-9]+(\.[0-9]+){3,4}'),'N/A')
              from (select patch_id, status, description
                      from dba_registry_sqlpatch
                     where action = 'APPLY'
                       and upper(description) like 'DATABASE RELEASE UPDATE%'
                     order by action_time desc)
             where rownum = 1),'N/A|N/A|N/A') || '|' ||
       nvl((select to_char(patch_id) || '|' || status
              from (select patch_id, status, description
                      from dba_registry_sqlpatch
                     where action = 'APPLY'
                       and upper(description) like '%OJVM%'
                     order by action_time desc)
             where rownum = 1),'N/A|N/A') || '|' ||
       nvl((select to_char(action_time,'YYYY-MM-DD HH24:MI')
              from (select action_time
                      from dba_registry_sqlpatch
                     order by action_time desc)
             where rownum = 1),'N/A') || '|' ||
       nvl((select to_char(count(*)) from dba_objects where status <> 'VALID'),'0')
from v\$database;
exit;
EOF
) || true

  sqlpatch_line=$(echo "$sqlpatch_line" | grep '|' | tail -1 | sed 's/^ *//;s/ *$//')

  if [[ -n "$sqlpatch_line" ]]; then
    IFS='|' read -r db_role open_mode db_version compatible ru_patch ru_status ru_level ojvm_patch ojvm_status last_patch_time invalid_count <<< "$sqlpatch_line"
    sql_status="SUCCESS"
  else
    sql_status="FAILED"
    patch_result="FAILED"
    log_output "PATCH REPORT SQL ERROR - $dbName" "$sqlpatch_line"
  fi

  # Overall compliance result.
  if [[ "$sql_status" == "FAILED" || "$ru_status" != "SUCCESS" || "$dp_version" == "NOT_FOUND" || "$opatch_version" == "NOT_FOUND" ]]; then
    patch_result="FAILED"
  elif [[ "$ojvm_status" != "SUCCESS" || "$invalid_count" != "0" || "$ru_patch" == "N/A" ]]; then
    patch_result="WARNING"
  else
    patch_result="PASS"
  fi
  ...
  case "$patch_result" in
    PASS) ((++PATCH_PASS_CNT)) ;;
    WARNING) ((++PATCH_WARNING_CNT)) ;;
    FAILED) ((++PATCH_FAILED_CNT)) ;;
  esac

  # Keep DB version compact for report readability, for example 19.31 from 19.31.0.0.0.
  local db_ver_short="$db_version"
  if [[ "$db_ver_short" =~ ^([0-9]+\.[0-9]+) ]]; then
    db_ver_short="${BASH_REMATCH[1]}"
  fi

  printf "%-4s %-12s %-17s %-14s %-8s %-13s %-11s %-11s %-11s %-13s %-13s %-16s %-8s %-6s\n" \
    "$MATCH_CNT" "$dbName" "$db_role" "$open_mode" "$db_ver_short" "$ru_level" "$ru_patch" "$ojvm_patch" "$opatch_version" "$dp_version" "$sql_status" "$last_patch_time" "$invalid_count" "$patch_result" \
    | tee -a "$LOG_FILE"
}
...
run_version_status() {
  ...
  version_line=$("$ORACLE_HOME/bin/sqlplus" -L -s /nolog <<EOF 2>&1
whenever sqlerror exit failure
connect sys/${syspass}@${connect_alias} as sysdba
set heading off feedback off pages 0 lines 600 trimspool on verify off echo off termout on
select database_role || '|' ||
       ...
       nvl((select value from v\$parameter where name='compatible'),'UNKNOWN') || '|' ||
       nvl((select to_char(patch_id) || '|' || status || '|' || nvl(regexp_substr(description,'[0-9]+(\.[0-9]+){3,4}'),'N/A')
              from (select patch_id, status, description, action_time
                      from dba_registry_sqlpatch
                     where action = 'APPLY'
                       and upper(description) like 'DATABASE RELEASE UPDATE%'
                     order by action_time desc)
...
EOF
  ...
  version_line=$(echo "$version_line" | grep '|' | tail -1 | sed 's/^ *//;s/ *$//')

  if [[ -n "$version_line" ]]; then
    IFS='|' read -r db_role open_mode db_version compatible ru_patch ru_status ru_level ojvm_patch ojvm_status last_patch_time <<< "$version_line"
    sql_status="SUCCESS"
  else
    sql_status="FAILED"
    version_result="FAILED"
    log_output "VERSION STATUS SQL ERROR - $dbName" "$version_line"
  fi

  if [[ "$sql_status" == "FAILED" || "$ru_status" != "SUCCESS" || "$dp_version" == "NOT_FOUND" || "$opatch_version" == "NOT_FOUND" ]]; then
    version_result="FAILED"
  elif [[ "$ojvm_status" != "SUCCESS" || "$ru_patch" == "N/A" ]]; then
  ...
  local role_short="$db_role"
  [[ "$role_short" == "PHYSICAL STANDBY" ]] && role_short="STANDBY"

  printf "%-4s %-12s %-12s %-14s %-8s %-11s %-11s %-11s %-11s %-13s %-13s %-16s %-8s\n" \
    "$MATCH_CNT" "$dbName" "$role_short" "$open_mode" "$db_ver_short" "$compatible" "$ru_patch" "$ojvm_patch" "$opatch_version" "$dp_version" "$ru_status" "$last_patch_time" "$version_result" \
    | tee -a "$LOG_FILE"
}
...
process_db() {
  ...
  if [[ "$flagSkip" == "Y" && "$ACTION" != "STATUS" && "$ACTION" != "VERSION_STATUS" && "$ACTION" != "DATAPATCH_STATUS" && "$ACTION" != "PATCH_REPORT" ]]; then
    echo | tee -a "$LOG_FILE"
    echo " " | tee -a "$LOG_FILE"
  ...
  if [[ "$ACTION" != "STATUS" && "$ACTION" != "VERSION_STATUS" && "$ACTION" != "DATAPATCH_STATUS" && "$ACTION" != "PATCH_REPORT" ]]; then
    echo | tee -a "$LOG_FILE"
    echo " " | tee -a "$LOG_FILE"
    echo "Database     : $dbName" | tee -a "$LOG_FILE"
  ...
  case "$ACTION" in
    ...
    DATAPATCH_STATUS|PATCH_REPORT)
      local STATUS_CONNECT_ALIAS=""
      if [[ "$RUN_SIDE" == "PRIMARY" ]]; then
        STATUS_CONNECT_ALIAS="$dbUniqueName"
      else
        STATUS_CONNECT_ALIAS="$standbyDBUniqueName"
      fi
      run_datapatch_status "$dbHome" "$dbName" "$STATUS_CONNECT_ALIAS" "$syspass"
      ;;
    ...
    VERSION_STATUS)
      ...
      run_version_status "$dbHome" "$dbName" "$STATUS_CONNECT_ALIAS" "$syspass"
      ;;
...
show configuration;" "$DGMGRL_CONN" || true)"
      ...
      if [[ "${SWITCHOVER_SUCCESS:-N}" == "Y" ]]; then
        run_post_switchover_datapatch "$dbHome" "$dbName" "$SWITCH_TARGET_DB" "$syspass" || return 1
      else
        return 1
      fi
      ;;
...
if [[ "$ACTION" == "VERSION_STATUS" ]]; then
  echo | tee -a "$LOG_FILE"
  echo "                              ORACLE VERSION / PATCH STATUS" | tee -a "$LOG_FILE"
  printf '%0.s_' {1..155} | tee -a "$LOG_FILE"
  echo | tee -a "$LOG_FILE"
  printf "%-4s %-12s %-12s %-14s %-8s %-11s %-11s %-11s %-11s %-13s %-13s %-16s %-8s\n" \
    "SEQ" "DB_NAME" "ROLE" "OPEN_MODE" "DB_VER" "COMPATIBLE" "RU_PATCH" "OJVM_PATCH" "OPATCH" "DATAPATCH" "SQL_PATCH" "PATCH_DATE" "STATUS" \
    | tee -a "$LOG_FILE"
  printf '%0.s_' {1..155} | tee -a "$LOG_FILE"
  echo | tee -a "$LOG_FILE"
fi

if [[ "$ACTION" == "DATAPATCH_STATUS" || "$ACTION" == "PATCH_REPORT" ]]; then
  echo | tee -a "$LOG_FILE"
  echo "                              ORACLE PATCH COMPLIANCE REPORT" | tee -a "$LOG_FILE"
  printf '%0.s_' {1..170} | tee -a "$LOG_FILE"
  echo | tee -a "$LOG_FILE"
  printf "%-4s %-12s %-17s %-14s %-8s %-13s %-11s %-11s %-11s %-13s %-13s %-16s %-8s %-6s\n" \
    "SEQ" "DB_NAME" "ROLE" "OPEN_MODE" "DB_VER" "RU_LEVEL" "RU_PATCH" "OJVM_PATCH" "OPATCH" "DATAPATCH" "SQL_PATCH" "PATCH_DATE" "INVALID" "STATUS" \
    | tee -a "$LOG_FILE"
  printf '%0.s_' {1..170} | tee -a "$LOG_FILE"
  echo | tee -a "$LOG_FILE"
fi
...
print_summary() {
  [[ "$ACTION" == "STATUS" || "$ACTION" == "VERSION_STATUS" || "$ACTION" == "DATAPATCH_STATUS" || "$ACTION" == "PATCH_REPORT" ]] && return 0
  echo | tee -a "$LOG_FILE"
  echo " " | tee -a "$LOG_FILE"
...
' "SEQ" "DB_NAME" "RESULT" "TARGET" "NOTE" | tee -a "$LOG_FILE"
  printf '%0.s-' {1..155} | tee -a "$LOG_FILE"
  echo | tee -a "$LOG_FILE"
  local row
...
print_patch_report_summary() {
  [[ "$ACTION" != "VERSION_STATUS" && "$ACTION" != "DATAPATCH_STATUS" && "$ACTION" != "PATCH_REPORT" ]] && return 0
  echo | tee -a "$LOG_FILE"
  echo "SUMMARY" | tee -a "$LOG_FILE"
  printf '%0.s_' {1..155} | tee -a "$LOG_FILE"
  echo | tee -a "$LOG_FILE"
  echo "Databases Checked : $MATCH_CNT" | tee -a "$LOG_FILE"
  echo "PASS              : $PATCH_PASS_CNT" | tee -a "$LOG_FILE"
  echo "WARNING           : $PATCH_WARNING_CNT" | tee -a "$LOG_FILE"
  echo "FAILED            : $PATCH_FAILED_CNT" | tee -a "$LOG_FILE"
  echo | tee -a "$LOG_FILE"
  if [[ "$PATCH_FAILED_CNT" -gt 0 ]]; then
    echo "Overall Result    : FAILED" | tee -a "$LOG_FILE"
  elif [[ "$PATCH_WARNING_CNT" -gt 0 ]]; then
    echo "Overall Result    : WARNING" | tee -a "$LOG_FILE"
  else
